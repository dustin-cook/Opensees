function [ jnt ] = fn_joint_capacity( jnt, element, ele_prop_table, joint_TH )
...
if sum(strcmp('Vmax_2',column_low.Properties.VariableNames)) == 0 % Use capacities when demands do not yet exist
    ...
    % Estimation of shear demand based on surrounding element column demand
    ...
    if ~isempty(joint_TH)
        jnt.Vmax_force_control = jnt.Vmax;
        jnt.Mmax = max(abs(joint_TH.force_TH));
        jnt.Vmax = jnt.Mmax / (L/((L-jnt.w/2)/(0.9*jnt.height) - L/H)); % Reverse of Equation 2.1 of Hassan and Moehle 2012 (with toa*A taken as Vy)
    else
        jnt.Vmax_force_control = NaN;
        jnt.Mmax = NaN;
    end
end
