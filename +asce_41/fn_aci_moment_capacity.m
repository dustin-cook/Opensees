function [ Mu, Mn ] = fn_aci_moment_capacity( fc, b, d, P, As, As_d, fy, clear_cover, Es )
...
while abs(balance_eq(count)) > tolerance
    ...
    fs = min(e_s,fy/Es)*Es;
    balance_eq(count) = sum(As.*fs.*((As_d-c)./abs(As_d-c))) - P - 0.85*fc*b*0.85*c;
end
...
% Moment Capacity
Mn = sum(As.*fs.*abs(As_d-c)) + P*(d/2-c) + 0.85*fc*b*0.85*c*(c*1.15/2);
