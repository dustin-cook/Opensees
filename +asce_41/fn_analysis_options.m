...
% analysis.pushover_drift = 0.01; % Drfit limit where the pushover will go till
...
if strcmp(analysis.proceedure,'test')
    analysis.type_list = [2, 2]; % just one linear dynamic analysis
    analysis.nonlinear_list = [0, 1];
    analysis.dead_load_list = [1, 1];
    analysis.live_load_list = [1, 1];
    analysis.case_list = {'NA','NA'};
    analysis.pushover_drift_list = [0.01, 0.05];
elseif strcmp(analysis.proceedure,'NDP')
    analysis.type_list = [2, 2, 2, 2, 1]; % Linear Pushover then NL Pushover x 2 then 1 NL dynamic
    analysis.nonlinear_list = [0, 1, 1, 1, 1];
    analysis.dead_load_list = [1, 1, 1, 1, 1];
    analysis.live_load_list = [1, 1, 1, 1, 1];
    analysis.case_list = {'NA', 'NA', 'NA', 'backbones', 'NA'};
    analysis.pushover_drift_list = [0.01, 0.03, 0.03, 0.03, NaN]; % Drift limit where the pushover will go till
