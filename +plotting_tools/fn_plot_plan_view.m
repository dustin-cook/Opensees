function [ ] = fn_plot_plan_view( hinge, element, ele_prop_table, node, ele_side, plot_label, plot_dir )
...
%% B Value
for e = 1:height(ele_2_use)
    ele_props = ele_prop_table(ele_prop_table.id == ele_2_use.ele_id(e),:);
    ele_2_use.yeild_disp(e) = ele_2_use.(['Mn_pos_' num2str(ele_side)])(e)*ele_2_use.length(e)/(6*ele_props.e*ele_props.iz);
end
max_value = ceil(max(ele_2_use.(['b_hinge_' num2str(ele_side)]))*100)/100;
scatter_vals = ele_2_use.(['b_hinge_' num2str(ele_side)]) + ele_2_use.yeild_disp;
fn_plot_plan_scatter( node_2_use, scatter_vals, plot_dir, [plot_label, ' - B Value'], 0, plot_label, 'Total Deformation Limit (rad)', hinge_2_use.damage_recorded(strcmp(hinge_2_use.direction,'primary')), max_value )
